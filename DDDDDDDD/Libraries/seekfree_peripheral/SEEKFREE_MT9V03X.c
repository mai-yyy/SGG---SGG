...
int16 MT9V03X_CFG[CONFIG_FINISH][2]=
{
    ...
    {EXP_TIME,          450}, //曝光时间 500         摄像头收到后会自动计算出最大曝光时间，如果设置过大则设置为计算出来的最大曝光值450
    {FPS,              100},  //图像帧率 100         摄像头收到后会自动计算出最大FPS，如果过大则设置为计算出来的最大FPS0
    ...
    {UD_OFFSET,         40},   //图像上下偏移量    正值 上偏移   负值 下偏移  行为120 240 480时无法设置偏移    摄像头收偏移数据后会自动计算最大偏移，如果超出则设置计算出来的最大偏移
...
void seekfree_sendimg_my(UARTN_enum uartn, uint8 *image, uint16 width, uint16 height)
{
