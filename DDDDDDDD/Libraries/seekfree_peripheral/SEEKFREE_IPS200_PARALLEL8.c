...
IFX_ALIGN(4) IfxCpu_spinLock spinLock;
//-------------------------------------------------------------------------------------------------------------------
//  @brief      内部调用，用户无需关心
//  @param      void 		    
//  @return     				
...
//  @param      y               坐标y方向的起点  参数范围 0 - (IPS200_Y_MAX/16-1)
//  @param      dat             需要显示的字符串
// @param      front_color      前背景颜色
// @param      back_color      后背景颜色
//  @return     void
//  @since      v1.0
//  Sample usage:
//-------------------------------------------------------------------------------------------------------------------
void uicolorget(uint16 x,uint16 y,const int8 dat[],uint16 front_color,uint16 back_color)
{
    uint16 j;
    ...
    while(dat[j] != '\0')
    {
        ips200_myshowchar(x+8*j,y*16,dat[j],front_color,back_color);
        j++;
    }
}


void ips200_myshowchar(uint16 x,uint16 y,const int8 dat, uint16 front_color,uint16 back_color)
{
    uint8 i,j;
    uint8 temp;

    IfxCpu_setSpinLock(&spinLock, 0xffffff);
    for(i=0; i<16; i++)
    {
        ips200_address_set(x,y+i,x+7,y+i);
        temp = tft_ascii[(uint16)dat-32][i];//减32因为是取模是从空格开始取得 空格在ascii中序号是32
