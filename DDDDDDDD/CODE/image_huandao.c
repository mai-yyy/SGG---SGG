...
void Huandao_Sec_Low()
{
     ...
     LWhitePnt1 = real_char.mylinest[6];
      LWhitePnt2 = real_char.mylinest[10];
      LWhitePnt3 = real_char.mylinest[15];
      RWhitePnt1 = real_char.mylinest[145];
       RWhitePnt2 = real_char.mylinest[150];
       RWhitePnt3 = real_char.mylinest[154];
 uint8 R_diu_start=2;
 uint8 L_diu_start=2;
 uint8 L_out_count=0;
 uint8 L_out_start=0;
...
if(huan_L_cnt<=32&&huan_L_cnt>=15&&leftline[huan_L_fchange]!=0&&leftline[huan_L_fchange-3]!=0)
  {
    huan_L_flag=2;
...
//    huan_L_flag=3;}
    ...
    if(LWhitePnt1<=20&&LWhitePnt2<=20&&LWhitePnt3<=20)
    {
...
if(cyccy)
{
    ...
    for(int16 i=59;i>35;i--)
        ...
        if((over_run<=59&&over_run>=35&&icmdata.Yaw>=270)||icmdata.Yaw>=300)
                  {
//            buzzer(1);
            huan_L_flag = 5;
//            buzzer(1);
...
if((L_out_count>=59&&L_out_start>=55&&finalrun<=3)||huan_L_flag5_time>150||dt_code>11500)//小环11500
    {
  huan_L_flag=0;          //完全出环
  buzzer(0);
  bx_flag_4=0;
...
//  out_flag=44;
 ...
 //右环
if(!chuku_flag&&!huan_R_flag&&!huan_R_flag&&!qipao_flag&&!shizhiflag)   //确保在直道上才开始判断环岛
 {
              ...
              leftline[25]<=80&&leftline[20]<=80&&leftline[15]<=80&&leftline[10]<=80&&endline<=1&&left_all_lose_line==0)
    {
           ...
           if(leftline[i]!=0&&rightline[i]>=155&&leftline[i-1]!=0&&leftline[i-2]!=0)
           {
            ...
            if(huan_R_cnt>=18&&huan_R_cnt<=35&&rightline[huan_R_fchange]<=155&&rightline[huan_R_fchange-3]<=155)
              {
                  huan_R_flag=2;
//                  buzzer(1);
//                  out_flag=5;
...
//
    ...
    if(RWhitePnt1<=18&&RWhitePnt2<=18&&RWhitePnt3<=18)
      {
    ...
    if(RWhitePnt1>=30&&RWhitePnt2>=30&&RWhitePnt3>=30&&hz_r)
    {
    ...
    for(int i=57;i>endline;i--)
               {
                  ...
                  {
                      if(rightline[i]-rightline[i-1]<=-3&&image_data[58][151]==0&&image_data[56][151]==0&&image_data[54][151]==0)
                      {
                           Island_inflexion_row = i;
                           break;
                      }
...
if((R_out_count>=59&&R_out_start>=55&&finalrun_r<=3)||dt_code>12500)
//if(finalrun_r<=5)
    {
    buzzer(0);
    bx_flag_4_r=0;
